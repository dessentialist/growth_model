## Product Growth System – Implementation Plan (BPTK_Py SD + ABM, Stepwise Runner)
...
- Create an industry-agnostic system that can model any sectors and products by configuring the inputs.
...
- Deterministic execution order each step: instantiate agents → agent actions → aggregate product demand → set gateway values → advance SD → capture outputs.
...
- Capacity sharing: a single time-varying capacity per product constrains both anchor and direct client deliveries via a fulfillment ratio.
- Multi-phase, per-agent requirement generation: initial → ramp → steady, with per-sector growth rates and activation delays.
- Time-varying lookups: price and capacity are defined as lookup tables by product and read at the current simulation time.
- Sector–product attribution: anchor demand and deliveries must retain sector attribution for revenue by sector.
...
- Inputs
  - `Inputs/Lists.csv` (markets, sectors, products)
  - `Inputs/Anchor Client Parameters.csv` (per sector)
  - `Inputs/Other Client Parameters.csv` (per product)
  - `Inputs/Production Table.csv` (capacity by product over time)
  - `Inputs/Pricing Table.csv` (price by product over time)
- Tasks
  - Implement robust CSV loader with cleaning rules: strip currency/percent, force numeric, guard NaNs, and sort time-series.
  - Build canonical mappings: sectors → primary products, products → sectors (as needed), and master lists.
  - Validate coverage: every sector and product referenced anywhere must have all required parameters present.
...
- Tasks
  - Implement `create_element_name(base[, sector][, product])` that:
  ...
  - Provide helpers for canonical element names described in the architecture (e.g., `Anchor_Lead_Generation_<sector>`, `Price_<product>`).
- Success Criteria
...
- Testing
  - Unit tests for a representative set of sectors/products with spaces and special characters.
- Debugging
...
- Tasks
  - Build a `Model` with elements per product p:
    - Constants/converters for: `lead_start_year_<p>`, `inbound_lead_generation_rate_<p>`, `outbound_lead_generation_rate_<p>`, `lead_to_c_conversion_rate_<p>`, `lead_to_requirement_delay_<p>`, `requirement_to_fulfilment_delay_<p>`, `avg_order_quantity_initial_<p>`, `client_requirement_growth_<p>`, `TAM_product_<p>`.
    - Stocks/flows for leads and clients with accumulate-and-fire discrete client creation:
      - `Inbound_Leads_<p>`, `Outbound_Leads_<p>`, `Total_New_Leads_<p>`
      - `Potential_Clients_<p>` (accumulates fractional conversions)
      - `Client_Creation_<p>` (integer fire) and `C_<p>` (cumulative clients)
    ...
    - Total demand and fulfillment ratio:
      - `Agent_Demand_Sector_Input_<s>_<p>` (converter per sector–product; numeric; set each step by runner)
      - `Agent_Aggregated_Demand_<p> = sum_s Agent_Demand_Sector_Input_<s>_<p>`
      - `Total_Demand_<p> = Agent_Aggregated_Demand_<p> + Client_Requirement_<p>`
      - `Fulfillment_Ratio_<p> = min(1, if Total_Demand_<p> > 0 then max_capacity_lookup_<p> / Total_Demand_<p> else 1)`
    - Deliveries & revenue:
      ...
      - `Client_Revenue_<p> = Client_Delivery_Flow_<p> * Price_<p>`
  - Ensure equations use quarters directly; no dt scaling inside delays/durations.
- Success Criteria
  - Model compiles; all required per-product elements exist with exact names; no missing parameter references.
  - With zero agents and valid direct-client inputs, the model runs and produces non-negative flows and revenues.
- Testing
  - Unit tests for lookup creation and `delay` usage (single-product micro scenario).
  - Golden integration test with 1 product and handcrafted small dataset; compare against hand calculations.
- Debugging
...
- Tasks
  - Implement `AnchorClientAgent` with fields: sector, activation_time, state, `requirements` dict keyed by product, counters for projects.
  - Implement `act(t, round_no, step_no)` to:
    ...
    - Generate per-product requirements using initial→ramp→steady phase logic with per-sector parameters and product start years.
...
- Debugging
  - Add DEBUG logs in first N steps showing state changes and per-product requirements for a few agents.
...
- Tasks
  - `ui/components/primary_map_editor.py`: sector selector, current mapping preview, proposed mapping builder (products multi‑select + per‑product `start_year`).
  - `ui/components/seeds_editor.py`: tabs for anchor sector seeds (hidden in SM‑mode), SM seeds editor (shown only in SM‑mode), and direct client seeds per product.
...
- Tasks
  - For each sector–product pair, create SD converter `Agent_Demand_Sector_Input_<s>_<p>`; compute `Agent_Aggregated_Demand_<p> = sum_s Agent_Demand_Sector_Input_<s>_<p>`; wire `Total_Demand_<p> = Agent_Aggregated_Demand_<p> + Client_Requirement_<p>`.
  - Compute anchor deliveries per sector with sector–product specific order lag L_{s,p} using SD constructs:
    ...
    - `Anchor_Delivery_Flow_<p> = sum_s Anchor_Delivery_Flow_<s>_<p>`
- Success Criteria
  - Gateways accept numeric equations and can be updated each step without re-compilation.
  - `Anchor_Delivery_Flow_<p>` reflects both lags and capacity via `Fulfillment_Ratio_<p>`.
- Testing
  - Integration test: inject synthetic agent demand and verify deliveries after the correct lag with capacity capping.
- Debugging
  - DEBUG print first N steps of `Agent_Demand_Input_<p>`, `Total_Demand_<p>`, `Fulfillment_Ratio_<p>`.
...
- Tasks
  ...
  - Per step:
    ...
    - Aggregate per-sector–product requirements across agents; set `Agent_Demand_Sector_Input_<s>_<p>` numeric equations accordingly.
...
- Success Criteria
  ...
  - Internal identity holds each step: `Total_Revenue ≈ sum_p(Anchor_Revenue_<p> + Client_Revenue_<p>)`.
- Testing
...
- Tasks
  - Map model elements to output rows exactly as specified (e.g., `Revenue`, `Revenue <sector>` where sector revenue is anchor-only, `Revenue <product>` which is anchor+direct, `Anchor Leads`, `Other Clients <product>`, `Order Basket <product>`, `Order Delivery <product>`).
...
- Success Criteria
  ...
  - Cross-check: `Order Delivery <p>` equals `Anchor_Delivery_Flow_<p> + Client_Delivery_Flow_<p>` per quarter.
- Testing
...
- Tasks
  ...
  - Step-level gateway sampling: log selected products for first 4–6 steps, mid-run, and last step.
...
- Data completeness: all required sector/product parameters present and numeric.
...
- Scope
  - Enhance scenario handling to make parameter toggling first‑class: constants and lookup points for all sectors/products.
...
- Tasks
  - Add `viz/plots.py` with functions:
    ...
    - `plot_order_basket_vs_delivery(df)` per product and totals
...
### Phase 13 — Multi‑Material Anchors (extend sector→products mapping) and Hardening
- Scope
  - Allow each sector to drive requirements for multiple products. SD side already supports per sector–product gateways; extend inputs and KPI enumeration.
- Tasks
  - `inputs.json`: extend `primary_map` entries to include multiple products per sector with `StartYear` per mapping.
  - `src/phase1_data.py`: validate that for every (sector, product) mapping, the product exists in other tables and `StartYear` is numeric (warn only if ≤ 0 per current policy).
  - `src/growth_model.py`:
    - Ensure creation of `Agent_Demand_Sector_Input_<s>_<p>` for every mapped pair.
    - Ensure `Anchor_Delivery_Flow_<s>_<p>` contributes to `Anchor_Delivery_Flow_<p>` and sector revenue aggregation.
  - `src/kpi_extractor.py`:
    - Enumerate KPI rows for any new products and maintain sector/product sums: `Revenue <p>` = `Anchor_Revenue_<p> + Client_Revenue_<p>`, `Revenue <s>` = sum over products’ sector revenues.
- Success criteria
  - When mapping adds products to a sector, CSV gains the corresponding rows; totals by product and sector reconcile every quarter.
  - No rows emit for unmapped pairs.
  - Verified via scenario `scenarios/multi_product_defense_aviation.yaml` and programmatic CSV checks: new product (Silicon Nitride Fiber) shows zero deliveries before 2026 and first non‑zero at 2027Q2 due to sector lag; `Revenue Defense` increases commensurately. Aviation with `Boron_fiber` mapped similarly.
- Testing
  - Unit: mapping validation (presence, numeric `StartYear`).
  - Integration: a crafted scenario where one sector maps two products with distinct lags/capacity; assert delivery timing and revenue attribution per sector–product and aggregation identities.
...
- Debugging
  - DEBUG snapshot for first few steps: per sector–product gateway values, fulfillment ratio, and anchor delivery flow; include step timestamps.
...
- Tasks
  ...
  - Runner (`simulate_growth.py`):
    ...
    - Ensure seeded agents generate requirements at step 0 but respect sector–product start years.
...
- Success criteria
  - With seeds configured, step‑0 KPIs reflect active counts; requirements and deliveries follow from seeded agents respecting sector–product start years and lags. ATAM gating includes seeded clients via `CPC_<sector>` initialization.
- Testing
  - Unit: scenario validation for seeds; agent factory creates ACTIVE agents with expected initial state.
  - Integration: compare runs with and without seeds; assert non‑zero early requirements/deliveries only for seeded sectors/products after appropriate lags.
- Debugging
  - DEBUG: log seed instantiation per sector (counts) and first‑step per‑product requirements from seeded agents.
...
## Phase 16 — Anchor Sector‑Product (SM) Parameters and SM Lists

### Scope
- Introduce full per‑(sector, product) parameters for Anchor (not for Direct Clients). Direct Clients remain product‑driven as before.
- Establish explicit SM lists (the universe of (s,p) pairs) to back per‑(s,p) inputs and validation. Initial SM list can be derived from the existing `primary_map` as a best‑guess starting point.

### Rationale
- Today, most anchor parameters are sector‑only while deliveries are realized per (s,p) via SD. This change lets us specify phase requirement parameters and anchor order lag at (s,p) granularity where needed, while preserving sector‑only and product‑only data where appropriate.

### Inputs (new/updated)
- `inputs.json` (new section)
  - `anchor_params_sm`: nested mapping param → sector → product → value.
    - Targeted per‑(s,p) params (initial set):
...
- `inputs.json` (new/clarified)
  - `lists_sm`: optional explicit SM list with rows like `{ "Sector": "Defense", "Product": "Silicon Carbide Fiber" }`.
    - If absent, derive SM pairs from `primary_map` for this phase.

### Tasks
- Naming utilities (`src/naming.py`)
  - Add `anchor_constant_sm(param, sector, product)` helper to produce canonical names for per‑(s,p) constants (e.g., `requirement_to_order_lag_Defense_Silicon_Carbide_Fiber`).
- Phase 1 parsing & validation (`src/phase1_data.py`)
  - Parse `anchor_params_sm` into a normalized long table with columns [Sector, Product, Param, Value].
  ...
  - Validation:
    - Ensure any (s,p) entries reference existing sectors/products.
    - Fallback policy enforced in build (no hard requirement that all (s,p) params be present).
- Scenario loader (`src/scenario_loader.py`)
  - Expand permissible constants to include per‑(s,p) names generated via `anchor_constant_sm` for targeted params across the SM universe (from `lists_sm` or `primary_map`).
  - Maintain strict key validation and nearest‑match hints.
- SD model build (`src/growth_model.py`)
  - Create constants per (s,p) for targeted params with precedence: (s,p) value if provided, else sector-level.
  - Use `requirement_to_order_lag_<s>_<p>` in anchor deliveries; it always exists after build (value from (s,p) or sector fallback).
  - No changes to capacity/price (product‑level) or direct‑client SD structure.
- ABM (`src/abm_anchor.py`)
  - Extend sector factory to pull per‑(s,p) requirement phase params for each mapped product.
  - Update `AnchorClientAgent` to use product‑specific phase params (initial/ramp/steady rate/growth) when ACTIVE via optional per-product maps.
  - Keep leads and project lifecycle sector‑level.
- Scenario overrides application (`apply_scenario_overrides`)
  - Existing by‑name constant assignment works once (s,p) constants exist; ensure tests cover (s,p) override cases.

### Compatibility & Precedence
- Fallback policy (strict and explicit): for each targeted per‑(s,p) parameter at pair (s,p):
  1) Use the per‑(s,p) value if provided.
  2) Else use the sector‑only value (for phase params, use sector baseline).
  3) Else raise a validation error naming the missing (s,p) parameter.
- Direct clients remain product‑level; no change to their inputs in this phase.

### Success Criteria
- Providing per‑(s,p) parameters modifies deliveries and revenues only for those pairs; sector‑only behavior matches legacy when per‑(s,p) values are absent.
- Validation catches missing or mis‑keyed (s,p) inputs with actionable messages.
- Scenario overrides can target specific (s,p) constants and take effect immediately.

### Testing
- Unit: parsing of `anchor_params_sm`; name generation; precedence resolution.
- Integration: crafted scenario where one sector maps two products with distinct per‑(s,p) phase parameters and lags; verify timing and magnitudes.
...
- On validation error, include the exact (sector, product, param) triple missing.

### Data Initialization (best‑guess SM lists)
- For this phase, derive `lists_sm` from `primary_map` (all (s,p) pairs present in mapping). This keeps effort low and aligned with current usage.
...
## Phase 17 — SM‑mode (Global Per‑(Sector, Product) Control)

This phase family enables a global “SM‑mode” where all anchor pipeline parameters become per‑(sector, product). When enabled, there are no fallbacks or simplifications: every targeted parameter must be present for every (s,p) pair, and sector‑level creation is disabled. Legacy sector‑mode remains available and unchanged.

### Phase 17.1 — Schema, Naming, and Loader (strict, no fallbacks)
- Scope
  - Introduce a global mode switch and enforce complete per‑(s,p) coverage for all anchor parameters when SM‑mode is active.
- Tasks
  - Scenario schema
    - Add `anchor_mode: "sm" | "sector"` under `runspecs` (default: `"sector"`).
    - Add `seeds.active_anchor_clients_sm: { <Sector>: { <Product>: <int> } }` for SM‑mode seeding.
  - Inputs (`inputs.json`)
    - Extend `anchor_params_sm` to support the full anchor set per (s,p):
    ...
    - Require `lists_sm` (array of rows `{Sector, Product}`) as the SM universe when SM‑mode is active. No derivation from `primary_map` in SM‑mode.
  - Naming (`src/naming.py`)
    - Use `anchor_constant_sm(param, sector, product)` for all per‑(s,p) constants.
    - Add helpers for per‑(s,p) SD creation signals in 17.2 (if not already present) using `create_element_name`.
  - Loader (`src/phase1_data.py`, `src/scenario_loader.py`)
    ...
    - When `anchor_mode = "sm"`:
      ...
      - Strict overrides: only per‑(s,p) constants accepted for anchor parameters; unknown keys are hard errors.
- Success Criteria
  - SM‑mode refuses to load unless all targeted per‑(s,p) parameters and seeds (when present) are complete.
...
- Testing
  ...
  - Unit: permissible constants surface includes per‑(s,p) only in SM‑mode for anchor parameters.
- Debugging
  - Log counts at INFO: pairs, targeted parameters, coverage (e.g., `112/112 ok`).
  - On failure, print first N missing (sector, product, param) triples.

### Phase 17.2 — SD Model: Per‑(s,m) Creation Pipelines (no sector creation in SM‑mode)
- Scope
  - Build one creation pipeline per (s,p) pair; do not build sector‑level creation when SM‑mode is active.
- Tasks (BPTK_Py SD‑DSL)
  - For each (s,p) in `lists_sm` create:
    - Constants for all per‑(s,p) anchor parameters listed in 17.1.
    - Lead generation and PC conversion:
      - `Anchor_Lead_Generation_<s>_<p> = 4 * anchor_lead_generation_rate_<s>_<p> * If(Time ≥ anchor_start_year_<s>_<p>, 1, 0) * If(CPC_<s>_<p> < ATAM_<s>_<p>, 1, 0)`
      - `CPC_<s>_<p>` stock integrates the converter above.
      - `New_PC_Flow_<s>_<p> = Anchor_Lead_Generation_<s>_<p> * lead_to_pc_conversion_rate_<s>_<p>`
    - Accumulate‑and‑fire integerization (quarter‑correct):
      - `Agent_Creation_Accumulator_<s>_<p>` stock
      - `Agent_Creation_Inflow_<s>_<p> = New_PC_Flow_<s>_<p>`
      - `Agent_Creation_Outflow_<s>_<p> = max(0, Round(Agent_Creation_Accumulator_<s>_<p> - 0.5, 0))`
      - `Agent_Creation_Drain_<s>_<p> = 4 * Agent_Creation_Outflow_<s>_<p>`
      - `Agents_To_Create_<s>_<p> = Agent_Creation_Outflow_<s>_<p>`
      - `Cumulative_Agents_Created_<s>_<p>` stock with inflow `Agent_Creation_Drain_<s>_<p>`
  - Exclusivity: When `anchor_mode = "sm"`, do not create any sector‑level creation elements.
  - Deliveries side remains per‑(s,p) using `requirement_to_order_lag_<s>_<p>` as already implemented.
- Success Criteria
  - Model compiles with per‑(s,p) creation signals for all pairs; sector‑level creation is absent in SM‑mode.
- Testing
...
- Scope
  - Introduce `AnchorClientAgentSM` bound to a single (sector, product) with a self‑contained lifecycle.
- Tasks
  - Implement `AnchorClientAgentSM` with states POTENTIAL → PENDING_ACTIVATION → ACTIVE.
  - Use per‑(s,p) lifecycle params: `project_generation_rate`, `max_projects_per_pc`, `project_duration`, `projects_to_client_conversion`, `anchor_client_activation_delay`, phase durations, and per‑(s,p) requirement phase parameters.
  - Respect `anchor_start_year_<s>_<p>` gating and generate requirements only when ACTIVE.
  - Provide `build_sm_agent_factory(sector, product)` returning fresh SM agents.
- Success Criteria
  - Deterministic requirements per step; no sector‑level values referenced when in SM‑mode.
- Testing
  - Unit: lifecycle transitions, activation timing, phase behavior with per‑(s,p) params.
...
- Implemented `AnchorClientAgentSM` bound to a single (sector, product) with POTENTIAL → PENDING_ACTIVATION → ACTIVE lifecycle and phase‑based requirements gated by `anchor_start_year_<s>_<p>`.
- Implemented `build_sm_anchor_agent_factory(sector, product)` sourcing parameters strictly from `anchor_params_sm` (no sector fallbacks in SM‑mode).
...
- Scope
  - Switch the runner to use per‑(s,p) creation signals and SM agents when SM‑mode is active.
- Tasks
  - Step loop (SM‑mode):
    - Evaluate `Agents_To_Create_<s>_<p>` and instantiate via `build_sm_agent_factory`.
    - Call `.act(...)` on all SM agents; aggregate their single‑product requirements into `Agent_Demand_Sector_Input_<s>_<p>`.
  - Seeding (SM‑mode only):
    - `seeds.active_anchor_clients_sm[sector][product]` → create that many ACTIVE SM agents at t0.
    - Initialize SD stocks: `CPC_<s>_<p> = seeds_sm` and `Cumulative_Agents_Created_<s>_<p> = seeds_sm` if elements exist; log non‑fatals otherwise.
  - Prohibit legacy sector‑level seeding in SM‑mode (raise if both present).
- Success Criteria
  - End‑to‑end SM‑mode run completes; KPIs produced; per‑(s,p) seeds visible at step 0.
- Testing
...
Implementation status
- Runner switches to SM‑mode when `runspecs.anchor_mode == "sm"`, instantiating agents via `build_sm_anchor_agent_factory` using per‑(s,p) creation signals `Agents_To_Create_<s>_<p>`.
- SM‑mode seeding is supported via `seeds.active_anchor_clients_sm`, creating ACTIVE agents at t0 and initializing `CPC_<s>_<p>` and `Cumulative_Agents_Created_<s>_<p>` when elements exist. Sector‑level seeding remains prohibited in SM‑mode (enforced by loader).
- Per‑step aggregation writes `Agent_Demand_Sector_Input_<s>_<p>` for all mapped pairs with deterministic zeroing when absent.
- KPI capture sums per‑(s,p) `Anchor_Lead_Generation_<s>_<p>` when sector‑level `Anchor_Lead_Generation_<s>` is absent in SM‑mode to compute `Anchor Leads <sector>`.
- Validation added for SM creation signals: `validate_agents_to_create_sm_signals` ensures non‑negative integer `Agents_To_Create_<s>_<p>` each step.

### Phase 17.5 — Strict Enforcement (No Fallbacks, No Simplifications)
- Scope
  - Guarantee strict inputs‑only policy in SM‑mode.
- Tasks
  - Loader/model: require full per‑(s,p) coverage; raise on any missing (s,p,param). Implemented in `src/scenario_loader.py` (SM completeness + explicit `lists_sm`) and `src/growth_model.py` (strict `_require_anchor_sm_value` paths when `runspecs.anchor_mode == "sm"`).
  - Scenario overrides: accept only per‑(s,p) anchor constants in SM‑mode. Implemented by mode-specific permissible constants surface in `src/scenario_loader.py`.
...
- Testing
  - Negative: partial `anchor_params_sm` → error; sector seeds in SM‑mode → error; unknown per‑(s,p) keys → error. Implemented in `tests/test_phase17_5_strict.py`.
- Debugging
  - Echo compliance counts; list first N issues precisely.

### Phase 17.6 — KPIs and Optional Granularity
- Scope
  - Keep KPI surface stable; optionally add granular per‑(s,p) diagnostics.
- Tasks
  - Default KPI mapping unchanged (sector/product totals intact).
  - Optional (flag): emit `Anchor Clients <sector> <product>` and/or `Revenue <sector> <product>`.
...
- Tasks
  ...
  - Schema (SM mode): `seeds.completed_projects_sm: { <Sector>: { <Product>: <int> } }`.

...
def _capture_step_kpis(
    *,
    ...
    for m in products:
        ...
        # Production capacity per material is quarterly (model divides by 4 at build)
        cap_m = _safe_eval(model, max_capacity_converter_product(m), t)

        out[f"Order Basket {m}"] = td
        out[f"Order Delivery {m}"] = adf_m + cdf
        out[f"Revenue {m}"] = (adf_m + cdf) * price
        out[f"Production Capacity {m}"] = cap_m
        # Capacity utilization per material with safe fallback and clipping
        denom_m = cap_m if cap_m > 0 else (td if td > 0 else 1e-9)
        util_m = (adf_m + cdf) / denom_m
        if util_m < 0.0:
            util_m = 0.0
        if util_m > 1.0:
            util_m = 1.0
        out[f"Capacity Utilization {m}"] = float(util_m)
    ...
    # Production Capacity total (sum of per-material quarterly capacities)
    out["Production Capacity"] = sum(out.get(f"Production Capacity {m}", 0.0) for m in products)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Base name for the timestamped CSV copy (without extension). If omitted, uses scenario name.",
    )
    return p.parse_args()
...
def run_stepwise(
    ...
    *,
        ...
        for sector, mat_map in seeds_active_sm.items():
            for material, k in mat_map.items():
                ...
                if factory is None:
                    ...
                    continue
                # Use SM-mode elapsed quarters for proper aging calculation
    ...
    for step_idx in range(num_steps):
        ...
        # Add a concise utilization debug snapshot
        try:
            delivery_total = float(step_kpis.get("Order Delivery", 0.0))
            capacity_total = float(step_kpis.get("Production Capacity", 0.0))
            denom = capacity_total if capacity_total > 0 else float(step_kpis.get("Order Basket", 0.0))
            util = (delivery_total / denom) if denom > 0 else 0.0
            log.debug(
                "util snapshot t=%.2f step=%d delivery=%.3f capacity=%.3f basket=%.3f util=%.3f",
                t,
                step_idx,
                delivery_total,
                capacity_total,
                float(step_kpis.get("Order Basket", 0.0)),
                util,
            )
        except Exception:
            pass
        ...
        if step_idx < 4 or step_idx in (num_steps // 2, num_steps - 1):
            ...
            # Cohort aging-chain diagnostics (optional, per product)
            # For products with use_cohort_buckets_<m> >= 0.5, log first few cohort buckets
            for m in bundle.lists.products[:2]:
                try:
                    B = float(model.evaluate_equation(product_constant("avg_order_quantity_initial", m), t))
                except Exception:
                    B = 0.0
                try:
                    g = float(model.evaluate_equation(product_constant("client_requirement_growth", m), t))
                except Exception:
                    g = 0.0
                try:
                    L = float(model.evaluate_equation(product_constant("requirement_limit_multiplier", m), t))
                except Exception:
                    L = 10.0
                per_cap = B * L
                # Log up to 6 ages or number of steps, whichever is smaller
                max_ages = min(6, num_steps)
                parts = []
                sum_cohort_orders = 0.0
                for age in range(max_ages):
                    cname = f"Direct_Client_Cohort_{age}_{str(m).replace(' ', '_')}"
                    try:
                        size = float(model.evaluate_equation(cname, t))
                    except Exception:
                        break
                    per_client_linear = B + (B * g) * float(age)
                    per_client_order = per_client_linear if per_client_linear <= per_cap else per_cap
                    sum_cohort_orders += size * per_client_order
                    parts.append(f"age{age}: n={size:.3f} per={per_client_order:.3f}")
                # Post-delay requirement value
                try:
                    req_name = f"Client_Requirement_{str(m).replace(' ', '_')}"
                    req_val = float(model.evaluate_equation(req_name, t))
                except Exception:
                    req_val = 0.0
                if parts:
                    log.debug(
                        "t=%.2f %s Cohorts :: %s || sum_pre_delay=%.6f, post_delay_requirement=%.6f",
                        t,
                        m,
                        " | ".join(parts),
                        sum_cohort_orders,
    ...
    # Phase 11/15 improvement: also write a timestamped copy for comparisons
    # Keep default file for regression tests; add a second file with timestamp and chosen base name.
    try:
        # Determine base name: CLI --output-name or scenario name
        try:
            # Access CLI args from outer scope if available
            import sys as _sys
            _argv = list(getattr(_sys, "argv", []))
        except Exception:
            _argv = []
        # Fallback parse to see if --output-name is present
        base_name = None
        if "--output-name" in _argv:
            try:
                idx = _argv.index("--output-name")
                base_name = _argv[idx + 1]
            except Exception:
                base_name = None
        if not base_name:
            base_name = str(scenario.name)
        base_name = re.sub(r"[^0-9A-Za-z_]+", "_", base_name.strip().replace(" ", "_"))
        ts = time.strftime("%y%m%d%H%M%S", time.localtime())
        timestamped = output_path.with_name(f"{base_name}_{ts}{output_path.suffix}")
        shutil.copyfile(output_path, timestamped)
        log.info("Also wrote timestamped KPI CSV to %s", timestamped)
    except Exception as _e:
        # Non-fatal; default output already written
        log.debug("Could not write timestamped KPI CSV: %s", _e)

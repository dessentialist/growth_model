...
with st.sidebar:
    ...
    run_current = st.button("Validate, Save, and Run", help="Validates current editor scenario, saves to scenarios/, and runs it")
...
if run_current:
	...
	if not ok:
		...
		rc = build_runner_command(scenario_path=path, debug=debug, visualize=visualize,
			kpi_sm_revenue_rows=kpi_sm_revenue_rows, kpi_sm_client_rows=kpi_sm_client_rows)
		ret = _run_and_render_logs(rc)
		st.success(f"Run finished with exit code {ret}")
		csv_path = find_latest_results_csv()
		with results_container:
...
st.info("Use the tabs to edit runspecs, constants, points, primary map, and seeds. Validate & Save writes YAML; sidebar can run presets or the current scenario.")
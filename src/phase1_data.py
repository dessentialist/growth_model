...
- Configurable market scope: we reconstruct lists under the default market specified
  in the defaults section, with fallback to the first available market.
"""
...
def parse_json_to_bundle(data: dict) -> "Phase1Bundle":
    ...
    # Get default market from defaults section, fallback to first market
    default_market = data.get("defaults", {}).get("default_market")
    if not default_market or default_market not in markets:
        if markets:
            default_market = markets[0]
            log.warning("Default market not found or not specified; using first market: %s", default_market)
        else:
            default_market = "Market_1"
            log.warning("No markets found in lists; using fallback market: %s", default_market)
    
    target_market = default_market
    log.debug("Using target market: %s", target_market)
    rows_l = [
...
def validate_coverage(bundle: Phase1Bundle) -> None:
    ...
    # Market validation for Phase 1 lists reconstruction
    markets_present = set(bundle.lists.markets_sectors_materials.get("Market", []).unique())
    if not markets_present:
        raise ValueError("No markets found in reconstructed lists — Phase 1 requires at least one market")
...
class Phase1Bundle:
    ...
    # Defaults section for UI fallback values

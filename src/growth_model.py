...
def _build_product_block(
    ...
    *,
    ...
    # ---- Client Requirement with a delay (quarters, no dt scaling) ----
    req_name = client_requirement(material)
    req = model.converter(req_name)
    # Compose input as an explicit converter to satisfy delay() Element constraint
    c_times_avg_name = f"C_times_avg_{material.replace(' ', '_')}"
    c_times_avg = model.converter(c_times_avg_name)
    c_times_avg.equation = c_clients * avg
    elements[c_times_avg_name] = c_times_avg
    req.equation = F.delay(model, c_times_avg, delay_lr)
    ...
    cdf.equation = F.delay(model, dcd, delay_cf)

...
class AnchorClientParams:
    ...
    # Requirement growth limit (percentage multiplier of initial rate)
    requirement_limit_multiplier: float  # e.g., 10.0 = 1000% of initial rate
...
class AnchorClientAgent:
    ...
    def __init__(
        ...
        *,
        ...
        # NEW: Override parameters for phase transitions (0 = use inherited value)
        sm_ramp_requirement_rate_override: Optional[Mapping[str, float]] = None,
        sm_steady_requirement_rate_override: Optional[Mapping[str, float]] = None,
        ...
        # NEW: Store override parameters (0 = use inherited value from previous phase)
        self._sm_ramp_requirement_rate_override = {str(k): float(v) for k, v in (dict(sm_ramp_requirement_rate_override or {})).items()}
        self._sm_steady_requirement_rate_override = {str(k): float(v) for k, v in (dict(sm_steady_requirement_rate_override or {})).items()}
    ...
    def _generate_requirements_for_current_step(self, t_years: float) -> Dict[str, float]:
        ...
        def phase_value_for_material(m: str, q_index: int) -> float:
            ...
            # NEW: Get override parameters (0 = use inherited value)
            ramp_override = self._sm_ramp_requirement_rate_override.get(m, 0.0)
            steady_override = self._sm_steady_requirement_rate_override.get(m, 0.0)
            
            # Calculate the requirement limit for this material (based on initial rate)
            requirement_limit = init_rate * self.params.requirement_limit_multiplier

            if q_index < d_initial:
                # Initial phase: apply limit to growth
                calculated_value = init_rate * (1 + init_g) ** q_index
                return min(calculated_value, requirement_limit)
            
            if q_index < d_initial + d_ramp:
                # Ramp phase: use override if provided, otherwise inherit from initial phase end
                step_in_ramp = q_index - d_initial
                if ramp_override > 0:
                    # Use explicit override value
                    calculated_value = ramp_override * (1 + ramp_g) ** step_in_ramp
                else:
                    # Inherit from initial phase ending value
                    initial_ending_value = init_rate * (1 + init_g) ** (d_initial - 1)
                    calculated_value = initial_ending_value * (1 + ramp_g) ** step_in_ramp
                return min(calculated_value, requirement_limit)
            
            # Steady phase: use override if provided, otherwise inherit from ramp phase end
            step_in_steady = q_index - (d_initial + d_ramp)
            if steady_override > 0:
                # Use explicit override value
                calculated_value = steady_override * (1 + steady_g) ** step_in_steady
            else:
                # Inherit from ramp phase ending value
                if ramp_override > 0:
                    # Ramp phase used override, so use its ending value
                    ramp_ending_value = ramp_override * (1 + ramp_g) ** (d_ramp - 1)
                else:
                    # Ramp phase inherited from initial, so calculate its ending value
                    initial_ending_value = init_rate * (1 + init_g) ** (d_initial - 1)
                    ramp_ending_value = initial_ending_value * (1 + ramp_g) ** (d_ramp - 1)
                calculated_value = ramp_ending_value * (1 + steady_g) ** step_in_steady
            return min(calculated_value, requirement_limit)
...
def build_anchor_agent_factory_for_sector(bundle: Phase1Bundle, sector: str) -> Callable[[], AnchorClientAgent]:
    ...
    params = AnchorClientParams(
        ...
        ramp_requirement_rate=0.0,  # Not used - per-(s,m) values come from sm_maps
        ramp_req_growth=_require_anchor_param(bundle, sector, "ramp_req_growth"),
        steady_requirement_rate=0.0,  # Not used - per-(s,m) values come from sm_maps
        steady_req_growth=_require_anchor_param(bundle, sector, "steady_req_growth"),
        requirement_limit_multiplier=_require_anchor_param(bundle, sector, "requirement_limit_multiplier"),
    )
    ...
    sm_maps = {
        ...
        "steady_req_growth": {},
        # NEW: Override parameters for phase transitions
    ...
    def factory() -> AnchorClientAgent:
        return AnchorClientAgent(
            ...
            # NEW: Pass override parameters (default to 0 for inherited behavior)
            sm_ramp_requirement_rate_override=sm_maps.get("ramp_requirement_rate_override", {}),
            sm_steady_requirement_rate_override=sm_maps.get("steady_requirement_rate_override", {}),
        )
...
class AnchorClientAgentSM:
    ...
    def __init__(
        ...
        *,
        ...
        # NEW: Override parameters for phase transitions (0 = use inherited value)
        ramp_requirement_rate_override: float = 0.0,
        steady_requirement_rate_override: float = 0.0,
        agent_label: Optional[str] = None,
    ) -> None:
        self.sector: str = str(sector)
        ...
        # NEW: Store override parameters (0 = use inherited value from previous phase)
        self.ramp_requirement_rate_override: float = float(ramp_requirement_rate_override)
        self.steady_requirement_rate_override: float = float(steady_requirement_rate_override)
    ...
    def _generate_requirement_for_current_step(self, t_years: float) -> float:
        ...
        # Calculate the requirement limit for this agent (based on initial rate)
        requirement_limit = self.params.initial_requirement_rate * self.params.requirement_limit_multiplier

        if q_since_activation < d_initial:
            # Initial phase: apply limit to growth
            calculated_value = self.params.initial_requirement_rate * (1 + self.params.initial_req_growth) ** q_since_activation
            return float(min(calculated_value, requirement_limit))

        if q_since_activation < d_initial + d_ramp:
            # Ramp phase: use override if provided, otherwise inherit from initial phase end
            step_in_ramp = q_since_activation - d_initial
            if self.ramp_requirement_rate_override > 0:
                # Use explicit override value
                calculated_value = self.ramp_requirement_rate_override * (1 + self.params.ramp_req_growth) ** step_in_ramp
            else:
                # Inherit from initial phase ending value
                initial_ending_value = self.params.initial_requirement_rate * (1 + self.params.initial_req_growth) ** (d_initial - 1)
                calculated_value = initial_ending_value * (1 + self.params.ramp_req_growth) ** step_in_ramp
            return float(min(calculated_value, requirement_limit))

        # Steady phase: use override if provided, otherwise inherit from ramp phase end
        step_in_steady = q_since_activation - (d_initial + d_ramp)
        if self.steady_requirement_rate_override > 0:
            # Use explicit override value
            calculated_value = self.steady_requirement_rate_override * (1 + self.params.steady_req_growth) ** step_in_steady
        else:
            # Inherit from ramp phase ending value
            if self.ramp_requirement_rate_override > 0:
                # Ramp phase used override, so use its ending value
                ramp_ending_value = self.ramp_requirement_rate_override * (1 + self.params.ramp_req_growth) ** (d_ramp - 1)
            else:
                # Ramp phase inherited from initial, so calculate its ending value
                initial_ending_value = self.params.initial_requirement_rate * (1 + self.params.initial_req_growth) ** (d_initial - 1)
                ramp_ending_value = initial_ending_value * (1 + self.params.ramp_req_growth) ** (d_ramp - 1)
            calculated_value = ramp_ending_value * (1 + self.params.steady_req_growth) ** step_in_steady
        return float(min(calculated_value, requirement_limit))
...
def build_sm_anchor_agent_factory(bundle: "Phase1Bundle", sector: str, material: str) -> Callable[[], AnchorClientAgentSM]:
    ...
    params = AnchorClientSMParams(
        ...
        requirement_limit_multiplier=float(_require_anchor_sm(bundle, sector, material, "requirement_limit_multiplier")),
    )

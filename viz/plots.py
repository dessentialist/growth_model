...
def plot_capacity_utilization(df: pd.DataFrame) -> Path:
    ...
    delivery = df.loc["Order Delivery", periods].astype(float)
    basket = df.loc["Order Basket", periods].astype(float).clip(lower=1e-9)
    util = (delivery / basket).clip(upper=1.0)

    x = range(len(periods))
    fig, ax = plt.subplots(figsize=(10, 4))
    ax.plot(x, util.values, label="Capacity Utilization (approx)")
    ax.set_title("Capacity Utilization (approximate)")
    ax.set_xticks(list(x))
    ax.set_xticklabels(periods, rotation=45, ha="right")
    ax.set_ylim(0, 1.05)
    ax.grid(True, alpha=0.3)

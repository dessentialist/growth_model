...
### Direct clients channel (per product p)
- Stocks
...
- Flows/Converters
  ...
  - `avg_order_quantity_<p> = avg_order_quantity_initial_<p> * (1 + client_requirement_growth_<p>)^(quarters_since_start)`
...
- Direct clients: average order quantity grows geometrically per quarter via `client_requirement_growth_<p>` producing within-client reinforcing demand growth (exogenous parameter driven)
...
flowchart LR
  ...
  subgraph Direct_Clients_SD
    ...
    CStock -- * avg_order_quantity_<p> --> Req[Client_Requirement_<p>]
...
- Direct client requirement: `delay(model, C_<p> * avg_order_quantity_<p>, lead_to_requirement_delay_<p>)`
